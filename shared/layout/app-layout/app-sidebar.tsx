...
const UserSidebar = ({ isCollapsed = false }: UserSideBarProps) => {
  ...
  return (
    ...
    >
      <ul className={classNames('relative flex flex-col w-[inherit] py-4 space-y-1', isCollapsed ? 'px-1' : 'px-2')}>
        <li className="truncate cursor-pointer" onClick={() => router.push(Routes.appHome)}>
        ...
        <li className="flex flex-col items-center justify-center py-10">
          ...
          <Body2 className={classNames('font-semibold', hideOnCollapseStyle)}>User</Body2>
        </li>
        ...
        <UserSidebarItem
          ...
          icon={HomeIcon}
          onClick={() => router.push(Routes.appHome)}
          selected={router.pathname == Routes.appHome}
        />
        <UserSidebarItem
          label="Esami"
          isCollapsed={isCollapsed}
          icon={ExamsIcon}
          onClick={() => router.push(Routes.appFindExams)}
          selected={router.pathname == Routes.appFindExams}
        />
        <UserSidebarItem
          label="Esci"
          isCollapsed={isCollapsed}
          icon={SignoutIcon}
          onClick={() => AuthService.signOutUser(router)}
        />
      </ul>
    </div>
  )
