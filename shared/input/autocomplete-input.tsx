...
type AutocompleteInputProps<T extends FieldValues> = React.HTMLProps<HTMLInputElement> & {
  errorMessage?: string
  options: string[]
...
const AutocompleteInput = <T extends FieldValues>({
  ...
  const optionsWithId = useMemo(() => options, [options])

  const [optionQuery, setOptionQuery] = useState('')
  const filteredOptions =
    optionQuery === ''
      ? optionsWithId
      : optionsWithId.filter((option) =>
        ...
        >
          <>
            <Combobox.Input
              ...
              displayValue={(option: typeof value) => option}
